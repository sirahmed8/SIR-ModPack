...
TRANSLATIONS = {
    ...
    "ar": {
        ...
        "hw_tier_ultra": "🌟 دقة سينمائية فائقة (جاهز لتتبع الأشعة وشاشات 4K)",
        "hw_tier_balanced": "⚡ أداء متوازن عالي الاستجابة (المعيار الموصى به)",
...
def detect_detailed_hardware():
    hw = {
        ...
        "recommended_ram": 6,
        "recommended_shader": "SIR Balanced Standard",
        "reason": "Optimal 6 GB RAM allocation calculated for stable high-refresh gaming."
    }
    try:
    ...
    if ram >= 24 or (ram >= 16 and any(k in gpu for k in ["4070", "4080", "4090", "3080", "3090", "7800", "7900", "XT"])):
        ...
        hw["recommended_shader"] = "SIR Balanced Standard"
        hw["reason"] = f"{ram} GB RAM & {hw['gpu_name']} detected. 6 GB RAM provides optimal frame-timing and shader fidelity."
    else:
        hw["tier"] = "comp"
        hw["tier_name"] = "🏆 COMPETITIVE TIER"
        hw["recommended_ram"] = 4
        hw["recommended_shader"] = "Internal / Fast"
        hw["reason"] = f"Tuned for maximum responsiveness, zero input lag, and fluid competitive play."
