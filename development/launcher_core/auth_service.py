...
class AuthService:
    ...
    def register_url_protocol(self) -> None:
        """Registers sirlauncher:// and sir-launcher:// protocols in Windows registry for deep linking."""
        if sys.platform != "win32":
            return
        try:
            import winreg
            exe_path = sys.executable if getattr(sys, "frozen", False) else sys.executable
            for proto in ("sirlauncher", "sir-launcher"):
                key_path = rf"Software\Classes\{proto}"
                with winreg.CreateKey(winreg.HKEY_CURRENT_USER, key_path) as key:
                    winreg.SetValueEx(key, "", 0, winreg.REG_SZ, "URL:SIR Launcher Protocol")
                    winreg.SetValueEx(key, "URL Protocol", 0, winreg.REG_SZ, "")
                
                cmd_key_path = rf"Software\Classes\{proto}\shell\open\command"
    ...
    def _normalize_legacy(self, source: dict[str, Any]) -> dict[str, Any] | None:
        ...
        access_token = str(source.get("accessToken") or source.get("access_token") or source.get("token") or "")
        refresh_token = str(source.get("refreshToken") or source.get("refresh_token") or "")
        profile_id = str(source.get("prismProfileId") or source.get("profileId") or profile.get("id") or "")
        
        # Real official accounts with tokens, yggdrasil data, or prism profile IDs
        has_auth_data = bool(access_token or refresh_token or profile_id or any(key in source for key in ("msa", "utoken", "xrp-mc", "ygg")))
        account_type = "microsoft" if (official_label and has_auth_data) else ("microsoft" if official_label else "offline")
    ...
    def _legacy_sources(self) -> list[str]:
        if os.environ.get("SIR_TESTING") or "temp" in self.data_dir.lower() or "tmp" in self.data_dir.lower():
            return []
    ...
    def _discover_system_accounts(self) -> list[dict[str, Any]]:
        """Automatically discovers Microsoft and active profiles from Lunar Client and Vanilla Minecraft Launcher."""
        if os.environ.get("SIR_TESTING") or "temp" in self.data_dir.lower() or "tmp" in self.data_dir.lower():
            return []
        discovered = []
        ...
        lunar_acc_path = os.path.join(user_home, ".lunarclient", "settings", "game", "accounts.json")
        if os.path.isfile(lunar_acc_path):
            try:
                with open(lunar_acc_path, "r", encoding="utf-8") as f:
                    ldata = json.load(f)
                accounts_dict = ldata.get("accounts", {})
                for aid, ainfo in accounts_dict.items():
                    name = str(ainfo.get("username") or "").strip()
                    if name:
                        acc_obj = self._public_account(
                            account_id=self._stable_id("microsoft", name),
                            name=name,
                            account_type="microsoft",
                            profile_id=ainfo.get("localId", aid),
                            uuid_value=ainfo.get("minecraftUuid", ""),
                            skin_url=f"https://mc-heads.net/skin/{name}",
                            model="classic",
                            access_token=ainfo.get("accessToken", "")
                        )
                        discovered.append(acc_obj)
            except Exception:
                pass

        # 2. Vanilla Minecraft Launcher Accounts
        vanilla_paths = [
        ...
        for vp in vanilla_paths:
            if os.path.isfile(vp):
                try:
                    with open(vp, "r", encoding="utf-8") as f:
                        vdata = json.load(f)
                    for aid, ainfo in vdata.get("accounts", {}).items():
                        p = ainfo.get("minecraftProfile", {})
                        name = str(p.get("name") or ainfo.get("username") or "").strip()
                        if name:
                            acc_obj = self._public_account(
                                account_id=self._stable_id("microsoft", name),
                                name=name,
    ...
    def load_accounts(self) -> list[dict[str, Any]]:
        ...
        # Auto-discover local system accounts from Lunar Client / Vanilla launcher
        system_discovered = self._discover_system_accounts()
        if system_discovered:
            existing_names = {a.get("displayName", "").lower() for a in accounts}
            for sys_acc in system_discovered:
                if sys_acc.get("displayName", "").lower() not in existing_names:
                    accounts.insert(0, sys_acc)
                    existing_names.add(sys_acc.get("displayName", "").lower())
            accounts = self._dedupe(accounts)
            self.save_accounts(accounts)

        if accounts or os.path.exists(self.migration_marker):
            return self._dedupe(accounts)

        # One-time migration from old executable-local stores.
        ...
        migrated = self._dedupe(migrated + accounts)
    ...
    def _dedupe(self, accounts: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for account in accounts:
            name_key = str(account.get("displayName", "")).strip().lower()
    ...
    def _public_account(self, *, account_id: str, name: str, account_type: str, profile_id: str, uuid_value: str, skin_url: str, model: str, needs_relink: bool = False, updated_at: str | None = None, access_token: str = "", refresh_token: str = "") -> dict[str, Any]:
    ...
    def save_accounts(self, accounts: list[dict[str, Any]] | None = None) -> None:
        ...
        if not self.active_account_id and self.accounts:
            self.active_account_id = self.accounts[0].get("accountId", "")
            self.active_account_name = self.accounts[0].get("displayName", "")
        atomic_write_json(
            self.accounts_file,
            {
    ...
    def _active_name(self) -> str:
        ...
        if not active and self.accounts:
            ...
            self.active_account_id = active.get("accountId", "")
    ...
    def add_offline_account(self, name: str, skin_url: str = "", model: str = "classic") -> dict[str, Any]:
        ...
        # Push to global Firebase RTDB
        if not os.environ.get("SIR_TESTING"):
            try:
                put_url = f"https://sir-modpack-default-rtdb.europe-west1.firebasedatabase.app/cloud_accounts/{urllib.parse.quote(name.lower())}.json"
                put_payload = json.dumps({
                    "name": name,
                    "ign": name,
                    "type": "offline",
                    "skinUrl": skin_url or f"https://mc-heads.net/skin/{name}",
                    "model": model,
                    "updatedAt": self._now()
                }).encode("utf-8")
                put_req = urllib.request.Request(put_url, data=put_payload, headers={"Content-Type": "application/json"}, method="PUT")
    ...
    def sync_cloud_accounts(self) -> dict[str, Any]:
        """Scoped native cloud synchronization protecting user account privacy."""
        added_count = 0
        try:
            # 1. Push user's authentic local accounts up to cloud
            if not os.environ.get("SIR_TESTING"):
                for acc in self.accounts:
                    acc_name = acc.get("displayName") or acc.get("name", "")
                    if acc_name:
                        try:
                            acc_type = "microsoft" if any(x in str(acc.get("accountType") or acc.get("type", "")).lower() for x in ["msa", "microsoft"]) else "offline"
                            put_url = f"https://sir-modpack-default-rtdb.europe-west1.firebasedatabase.app/cloud_accounts/{urllib.parse.quote(acc_name.lower())}.json"
                            put_payload = json.dumps({
                                "name": acc_name,
                                "ign": acc_name,
                                "type": acc_type,
                                "accountType": acc_type,
                                "skinUrl": acc.get("skinUrl") or f"https://mc-heads.net/skin/{acc_name}",
                                "model": acc.get("model", "classic"),
                                "uuid": acc.get("uuid", ""),
                                "updatedAt": self._now()
                            }).encode("utf-8")
                            put_req = urllib.request.Request(put_url, data=put_payload, headers={"Content-Type": "application/json"}, method="PUT")
            ...
            print("Cloud account sync error:", err)
            return {"success": False, "error": str(err), "accounts": self.accounts}
        
        return {"success": True, "added": added_count, "accounts": self.accounts, "active": self.active_account_name}

    def redeem_sync_code(self, code: str) -> dict[str, Any]:
        """Redeem a 6-digit sync code from website and activate account immediately."""
        clean_code = str(code or "").strip()
        if not clean_code or len(clean_code) < 6:
            return {"success": False, "error": "Invalid 6-digit code"}
        try:
            url = f"https://sir-modpack-default-rtdb.europe-west1.firebasedatabase.app/launcherSyncCodes/{urllib.parse.quote(clean_code)}.json"
            req = urllib.request.Request(url, headers={"User-Agent": "SIR-Launcher/1.0"})
            with urllib.request.urlopen(req, timeout=6) as resp:
                data = json.loads(resp.read().decode("utf-8"))
            if data and isinstance(data, dict):
                name = str(data.get("username") or data.get("name") or data.get("ign") or "").strip()
                if name:
                    skin_url = str(data.get("skinUrl") or f"https://mc-heads.net/skin/{name}")
                    model = str(data.get("model") or "classic")
                    res = self.add_offline_account(name, skin_url, model)
                    return {"success": True, "account": res.get("account"), "message": f"Linked & activated @{name} from web code!"}
    ...
    def remove_account(self, name_or_id: str) -> dict[str, Any]:
        ...
        # Delete from global Firebase RTDB cloud_accounts
        name = account.get("displayName")
        if name:
            try:
                del_url = f"https://sir-modpack-default-rtdb.europe-west1.firebasedatabase.app/cloud_accounts/{urllib.parse.quote(name.lower())}.json"
                del_req = urllib.request.Request(del_url, method="DELETE", headers={"User-Agent": "SIR-Launcher/1.0"})
                urllib.request.urlopen(del_req, timeout=4)
            except Exception as e:
                print("RTDB delete error:", e)
    ...
    def sync_to_ingame_ias(self) -> None:
        ...
        try:
            ...
            atomic_write_json(self.ias_file, payload)

            # Propagate ias_accounts.json to all sub-instance Minecraft directories
            instances_root = self.data_dir
            if os.path.exists(instances_root):
                for entry in os.listdir(instances_root):
                    entry_path = os.path.join(instances_root, entry)
                    if os.path.isdir(entry_path) and not entry.startswith("."):
                        mc_dir = os.path.join(entry_path, "minecraft")
                        if os.path.isdir(mc_dir):
                            atomic_write_json(os.path.join(mc_dir, "ias_accounts.json"), payload)
                        if os.path.exists(os.path.join(entry_path, "mmc-pack.json")) or os.path.exists(os.path.join(entry_path, "instance.cfg")):
                            atomic_write_json(os.path.join(entry_path, "ias_accounts.json"), payload)
        except Exception:
            pass
...
<head>
  ...
  <style>
    ...
    .btn { display: inline-block; margin-top: 18px; padding: 12px 28px; background: #00e5ff; color: #06090e; font-weight: 900; font-size: 13px; border-radius: 14px; text-decoration: none; box-shadow: 0 4px 20px rgba(0,229,255,0.3); transition: transform 0.2s ease; }
    .btn:hover { transform: scale(1.05); background: #38ef7d; }
  </style>
</head>
<body>
  <div class="card">
    <h2>✓ Microsoft Sign-in Complete!</h2>
    <p>Your official Minecraft profile has been authenticated. You can safely return to <strong>SIR Launcher</strong>.</p>
    <span class="badge">Connected to SIR Ecosystem</span>
    <div style="margin-top:20px;">
      <a href="javascript:void(0)" onclick="tryReturnToLauncher()" id="return-btn" class="btn">🚀 Return to SIR Launcher</a>
    </div>
    <p id="return-hint" style="margin-top:16px;font-size:12px;color:#38ef7d;">✓ Account Synced! Switch back to your SIR Launcher window.</p>
  </div>
  <script>
    function tryReturnToLauncher() {
      try { window.close(); } catch(e) {}
      const hint = document.getElementById('return-hint');
      if (hint) hint.innerText = "✓ Active in SIR Launcher! You may now close this browser tab.";
    }
  </script>
</body>
</html>"""
...
<body style="background:#06090e;color:#ff5555;font-family:sans-serif;text-align:center;padding:50px;">
  ...
  <p>{err_desc or err or 'Login cancelled'}</p>
                            ...
                            self.wfile.write(html.encode("utf-8"))
                            if isinstance(auth_svc_ref._ms_browser_state, dict):
                                auth_svc_ref._ms_browser_state["status"] = "error"
                                auth_svc_ref._ms_browser_state["error"] = err_desc or err or "Login cancelled"
                            else:
                                auth_svc_ref._ms_browser_state = {
                                    "status": "error",
                                    "error": err_desc or err or "Login cancelled",
