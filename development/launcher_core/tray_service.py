"""
tray_service.py — Native Windows System Tray Engine & Autostart Manager for SIR Launcher.

Features:
- Pystray notification area icon using SIR_Icon.ico.
- Context menu: Open Launcher, 1-Click Launch Active Profile, Settings, Check for Updates, Exit Completely.
- Default action (double click) restores and brings window to foreground using Win32 API.
- Windows Startup Auto-Launch Integration via HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run.
...
def is_windows_autostart_enabled() -> bool:
    """Checks whether SIR Launcher is registered in Windows HKCU Startup Run key."""
    if sys.platform != "win32":
        return False
    try:
        import winreg
        key_path = r"Software\Microsoft\Windows\CurrentVersion\Run"
        app_name = "SIR Launcher"
...
def set_windows_autostart(enabled: bool, exe_path: Optional[str] = None) -> bool:
    """Enables or disables SIR Launcher auto-start on Windows boot with --autostart --minimized flags."""
    if sys.platform != "win32":
        return False
    try:
        import winreg
            ...
            if enabled:
                if not exe_path:
                    if getattr(sys, "frozen", False):
                        ...
                        cand = os.path.join(root, "SIR Launcher", "SIR Launcher.exe")
                        if os.path.isfile(cand):
                            exe_path = cand
                        else:
                            exe_path = os.path.join(root, "SIR Launcher.exe")
                
                cmd_value = f'"{exe_path}" --autostart --minimized'
        ...
        print(f"[TrayService] Failed to set autostart: {ex}", file=sys.stderr)
        return False


class TrayService:
    """Manages the Windows System Tray icon, background event loop, and context menu."""

    _instance: Optional["TrayService"] = None
    ...
    def __init__(
        ...
        TrayService._instance = self

    @classmethod
    def get_instance(cls) -> Optional["TrayService"]:
        ...
        try:
            ...
            def _enum_cb(hwnd, _):
                ...
                if owner.value == pid:
                    ...
                    # Strict exclusion: Do not touch child or owned windows of launcher
                    ...
                    # Main window title
                    if win_text.startswith("SIR Launcher"):
                        return True
        ...
        with self._lock:
            ...
            try:
                ...
                def _on_launch(icon, item):
                    if self.bridge_api:
                        threading.Thread(target=self.bridge_api.launch_game, daemon=True).start()

                def _on_settings(icon, item):
                    self.restore_and_focus_window()
                    if self.window:
                        try:
                            self.window.evaluate_js("openSettingsModal('window');")
                        except Exception:
                            pass

                def _on_updates(icon, item):
                    self.restore_and_focus_window()
                    if self.window:
                        try:
                            self.window.evaluate_js("checkForAppUpdates();")
                        except Exception:
                            pass
                ...
                menu = pystray.Menu(
                    pystray.MenuItem("✦ Open SIR Launcher", _on_open, default=True),
                    pystray.MenuItem("⚡ Launch Active Profile (1-Click)", _on_launch),
                    pystray.Menu.SEPARATOR,
                    pystray.MenuItem("⚙ Launcher Settings", _on_settings),
                    pystray.MenuItem("🔄 Check for Updates", _on_updates),
                    pystray.Menu.SEPARATOR,
                ...
                print(f"[TrayService] Unable to initialize system tray: {e}", file=sys.stderr)
                return False
