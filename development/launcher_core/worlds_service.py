...
from shared_core.runtime import atomic_write_zip
...
class WorldsService:
    ...
    def _get_all_saves_directories(self, instance_id="26.2"):
        ...
        dirs = [
            ...
            os.path.join(self.root_dir, "instances", str(instance_id), "saves"),
            os.path.join(self.root_dir, "instances", str(instance_id), "minecraft", "saves"),
            os.path.join(self.root_dir, "instances", "26.2", "saves"),
            os.path.join(self.root_dir, "instances", "26.2", "minecraft", "saves"),
            os.path.join(self.root_dir, "instances", "26.2-ultra", "saves"),
            os.path.join(self.root_dir, "instances", "26.2-ultra", "minecraft", "saves"),
            os.path.join(self.root_dir, "instances", "1.8.9", "saves"),
        ...
        # Dynamically scan all instances subfolders for saves
        inst_parent = os.path.join(self.root_dir, "instances")
        if os.path.isdir(inst_parent):
            for sub in os.listdir(inst_parent):
                p1 = os.path.join(inst_parent, sub, "saves")
                p2 = os.path.join(inst_parent, sub, "minecraft", "saves")
                dirs.extend([p1, p2])
    ...
    def create_world_backup(self, world_folder_or_path, instance_id="26.2"):
        ...
        try:
            with atomic_write_zip(backup_dest) as zf:
                for root, _, files in os.walk(w_path):
                    for f in files:
    ...
    def delete_world(self, world_folder_or_path, instance_id="26.2"):
        """Permanently deletes the specified world directory."""
        w_path = world_folder_or_path
        if not os.path.exists(w_path):
            for s_dir in self._get_all_saves_directories(instance_id):
                cand = os.path.join(s_dir, world_folder_or_path)
        ...
        try:
            ...
            return {"success": True, "message": "World permanently deleted."}
        except Exception as e:
            return {"success": False, "error": str(e)}
