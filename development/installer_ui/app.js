...
async function safeInitHardware() {
  ...
  isSpecsLoading = true;
  try {
    if (typeof initHardwareSpecs === 'function') {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Polling loop for pywebview bridge injection
  let attempts = 0;
  const bridgeInterval = setInterval(() => {
    attempts++;
    if (window.pywebview && window.pywebview.api) {
      safeInitHardware();
      if (typeof checkPackageAvailability === 'function') checkPackageAvailability();
      if (STATE.specsFetched || attempts > 25) {
        clearInterval(bridgeInterval);
      }
    } else if (attempts > 30) {
      clearInterval(bridgeInterval);
      if (!STATE.specsFetched && typeof initHardwareSpecsFallback === 'function') {
