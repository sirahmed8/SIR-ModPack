...
def detect_system_java(version_hint=21) -> str:
    ...
    # Dynamic Program Files scan
    for pattern in [
        os.path.join(program_files, "Java", "*", "bin", "javaw.exe"),
        os.path.join(program_files, "Eclipse Adoptium", "*", "bin", "javaw.exe"),
        os.path.join(program_files, "Microsoft", "*", "bin", "javaw.exe"),
    ]:
        for matched in glob.glob(pattern):
            if os.path.isfile(matched):
                return os.path.abspath(matched).replace("\\", "/")

    # Check system PATH without spawning any console process
    which_javaw = shutil.which("javaw") or shutil.which("javaw.exe")
    if which_javaw and os.path.isfile(which_javaw):
        return os.path.abspath(which_javaw).replace("\\", "/")
