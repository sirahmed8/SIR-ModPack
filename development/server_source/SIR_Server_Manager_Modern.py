...
def main() -> None:
    ...
    tray = None
    try:
        from server_core import ServerTrayService
        tray = ServerTrayService(
            icon_path=icon_path,
            window=window,
            bridge_api=api,
            on_exit_callback=api.stop_server,
        )
    except Exception as e:
        print(f"[Server Manager] System tray service unavailable: {e}", file=sys.stderr)
    ...
    def on_closing():
        ...
        if action == "tray":
            ...
            try:
                ...
                pass
            try:
                if api.is_tunnel_running:
                    api.stop_playit_tunnel()
            except Exception:
                pass
            def _force_exit():
                time.sleep(0.2)
